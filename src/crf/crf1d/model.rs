...
impl Crf1dModel {
    ...
    pub fn from_memory(buffer: Vec<u8>) -> Self {
        ...
        let n_active_feats = u32::from_le_bytes(buffer[off_feats+8..off_feats+12].try_into().unwrap()) as usize;
        ...
        for i in 0..n_active_feats {
            ...
            let cat = u32::from_le_bytes(buffer[offset..offset+4].try_into().expect("failed to read cat"));
            ...
            let weight: f64 = f64::from_le_bytes(buffer[offset + 12..offset+20].try_into().unwrap());
            let f = Feature {
                cat: cat as usize,
                src: src as usize,
                dst: dst as usize,
        ...
        let n_active_label_refs = u32::from_le_bytes(buffer[off_label_refs+8..off_label_refs+12].try_into().unwrap()) as usize;
        println!("n: {}, n_act: {}", n_labels, n_active_label_refs);
        for i in 0..n_labels {
            let offset = off_label_refs + CHUNK_SIZE + 4 * i;
            let offset = u32::from_le_bytes(buffer[offset..offset+4].try_into().unwrap()) as usize;
            let n = u32::from_le_bytes(buffer[offset + 4..offset+8].try_into().unwrap()) as usize;
            let v = (0..n)
                .map(|j| {
                    u32::from_le_bytes(buffer[offset + 8 + 4 * j..offset+8+4*j+4].try_into().unwrap()) as usize
                })
                .collect::<Vec<_>>();
            label_refs.push(FeatRefs::new(n, v));
        }
        let mut attr_refs = Vec::new();
        let n_active_attr_refs = u32::from_le_bytes(buffer[off_attr_refs+8..off_attr_refs+12].try_into().unwrap()) as usize;
        ...
        let labels: Vec<(u32, String)> = CQDB::new(&buffer[off_labels as usize..]).expect("failed to read labels").iter().map(Result::unwrap).map(|(k,v)| (k, v.to_string())).collect();
        let attrs: Vec<(u32, String)> = CQDB::new(&buffer[off_attrs as usize..]).expect("failed to read attrs").iter().map(Result::unwrap).map(|(k,v)| (k, v.to_string())).collect();
